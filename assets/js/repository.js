...
function firebaseMessage(error){
  ...
  const message=String(error?.message||'Falha de comunicação com o Firebase.');
  return `${message}${code}`;
}
    ...
    if(admin&&includeSubmissions){
      await Promise.allSettled(quizzes.map(async quiz=>{
        try{
          ...
          quiz.submissions=submissionsSnapshot.docs
            .map(doc=>({id:doc.id,...doc.data()}))
            .sort((a,b)=>String(b.createdAt||'').localeCompare(String(a.createdAt||'')));
          quiz.statistics=quiz.statistics||{};
          quiz.statistics.totalResponses=quiz.submissions.length;
          if(quiz.submissions.length){
            quiz.statistics.averageScore=Math.round(quiz.submissions.reduce((sum,item)=>sum+(Number(item.score)||0),0)/quiz.submissions.length);
          }
        }catch(error){
...
export async function getQuizById(id){
  const all=await listQuizzes({admin:true,includeSubmissions:true});
...
export async function saveQuiz(quiz,{requireCloud=false}={}){
  ...
  if(requireCloud){
    if(!firebaseEnabled()){
      throw new Error('O Firebase não está ativo. O quiz não pode ser publicado para outros dispositivos.');
    }
    ...
    try{
      ...
      if(!saved){
        throw new Error('O Firebase não confirmou a existência do quiz após a publicação.');
      }
      if(saved.status!=='published'){
        throw new Error('O Firebase recebeu o quiz, mas o status publicado não foi confirmado.');
      }
      if(String(saved.slug||'').trim()!==String(quiz.slug||'').trim()){
        throw new Error('O Firebase confirmou um slug diferente do endereço do quiz.');
      }
  ...
  if(firebaseEnabled()){
    try{
      const f=await loadFirebase();
      await withTimeout(f.setDoc(f.doc(f.db,'quizzes',quiz.id),payload,{merge:true}),'Sincronização do quiz');
    }catch(error){
      console.warn('Alterações salvas localmente, mas ainda não sincronizadas com o Firebase.',error);
    }
  }
  return quiz;
}
...
export async function deleteQuiz(id){
  ...
  if(firebaseEnabled()){
    try{
      const f=await loadFirebase();
      await withTimeout(f.deleteDoc(f.doc(f.db,'quizzes',id)),'Exclusão no Firebase');
...
export async function saveSubmission(quiz,submission){
  ...
  if(firebaseEnabled()){
    try{
      const f=await loadFirebase();
      await withTimeout(f.setDoc(f.doc(f.db,'quizzes',quiz.id,'submissions',submission.id),deepClone(submission)),'Envio da resposta');
